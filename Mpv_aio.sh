...
if [ "$LANG_CHOICE" == "English" ]; then
    ...
    MSG_OSC_SUCCESS="✔️ Interface installed successfully!"
    MSG_OSC_SKIPPED="⚠️ Skipped interface installation."
    
    SUB_HEADER="📝 CHOOSE SUBTITLE TOOLS:"
    ...
    OSC_DESCRIPTIONS=(
        "The ultimate OSC with integrated menus"
    ...
    SUB_HEADER="📝 CHỌN CÔNG CỤ PHỤ ĐỀ:"
    ...
    OSC_DESCRIPTIONS=(
        "Giao diện tối thượng tích hợp Menu ngữ cảnh"
...
if [[ "$selected_osc" != "X."* && -n "$selected_osc" ]]; then
    ...
    # Dọn dẹp script và font cũ để CHỐNG XUNG ĐỘT
...
CONFLICTS=()
POTENTIAL_CONFLICTS=("uosc.lua" "osc.lua" "modernx.lua" "modernz.lua" "modernx_zydezu.lua" "osc-modern.lua" "progressbar.lua" "osc-orange.lua" "mfpbar.lua" "mpv_thumbnail_script.lua" "mpv_thumbnail_script_client_osc.lua" "osc_tethys.lua" "dark-box.lua" "light-box.lua" "modernx-compact.lua")
...
if [ ${#CONFLICTS[@]} -gt 0 ]; then
    echo "$CONFLICT_WARN"
    for item in "${CONFLICTS[@]}"; do echo "  - $item"; done
    echo ""
    if gum confirm "$CONFLICT_PROMPT"; then
        for item in "${CONFLICTS[@]}"; do rm -rf "$item"; done
